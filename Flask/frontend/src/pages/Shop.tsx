  ...
  useEffect(() => {
    const sendCompleteAnalytics = async () => {
      ...
      try {
        ...
        try {
          await apiClient.post('/user_analytics', analyticsPayload);
        ...
        if (currentShopMetrics.product_views.length > 0 || 
          ...
          try {
            await apiClient.post('/otp_attempts', {
              session_id: sessionId,
              risk_score: 10,
        ...
        const processActivityData = async () => {
          if (activityMetrics.current.productViews.size > 0 || 
            ...
            if (navigator.sendBeacon) {
              ...
              navigator.sendBeacon(`${import.meta.env.VITE_NEXT_PUBLIC_API_BASE_URL}/otp_attempts`, blob);
            }
          }
        };
