...
export const SessionActivityModal: React.FC<SessionActivityModalProps> = ({
  ...
  const fetchSessionActivity = async () => {
    ...
    try {
      ...
      const { data: analyticsData, error: analyticsError } = await apiClient.get('/user_analytics')
        .select('*')
      ...
      const { data: otpData, error: otpError } = await apiClient.get('/otp_attempts')
        .select('*')
        .eq('session_id', sessionId)
        .order('created_at', { ascending: false });
    ...
    data?.otpAttempts.forEach(attempt => {
      if (attempt.otp_code?.startsWith('SHOP_ACTIVITY_') && attempt.metadata) {
        shopActivities.push({
          ...
          email: attempt.metadata?.email || null, // Assuming email might be in otpAttempts metadata
    ...
    data?.analytics.forEach(record => {
      if (record.metadata?.shop_metrics) {
        ...
        shopActivities.push({
          ...
          email: record.metadata?.email || null,
                      ...
                      <Card key={index}>
                        <CardHeader>
                          ...
                          <CardDescription>
                            ...
                            {activity.email && <p>Email: {activity.email}</p>}
