import { apiClient } from '@/utils/api';
 ...
 export const testAnalyticsConnection = async () => {
  ...
  try {
    ...
    const { data: testData, error: testError } = await apiClient.get('/user_analytics')
      .select('count(*)')
      .limit(1);
    ...
    const { error: insertError } = await apiClient.post('/user_analytics')
      .insert({
    ...
    const { data: verifyData, error: verifyError } = await apiClient.get('/user_analytics')
      .select('*')
    ...
    await apiClient.delete('/user_analytics')
      .delete()
      .eq('session_id', testSessionId);
