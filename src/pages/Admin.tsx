...
import { adminService, analyticsService, authService } from '@/utils/flaskApi';
...
const AdminPage = () => {
  ...
  const fetchLoginAttempts = async () => {
    try {
      console.log('Fetching admin data from Flask API...');
      setIsLoading(true);
      setError(null);
      ...
      const [usersResult, auditLogsResult, dashboardStats] = await Promise.all([
        adminService.getUsers(),
        adminService.getAuditLogs(),
        analyticsService.getDashboardAnalytics()
      ]);

      // Convert Flask API data to match existing interfaces
      const attemptsData: LoginAttempt[] = auditLogsResult.logs.map(log => ({
        id: log.id,
        user_id: log.user_id,
        session_id: log.details?.session_id || 'unknown',
        risk_score: log.details?.risk_score || 0,
        otp_code: log.details?.otp_code || '',
        is_valid: log.action === 'login_success',
        ip_address: log.details?.ip_address || null,
        user_agent: log.details?.user_agent || null,
        created_at: log.timestamp,
        metadata: log.details
      }));

      const analyticsData: UserAnalytics[] = usersResult.users.map(user => ({
        id: user.id,
        user_id: user.id,
        session_id: 'current',
        page_url: '/dashboard',
        user_agent: null,
        typing_wpm: 45, // Default values - would come from actual analytics
        typing_keystrokes: 120,
        typing_corrections: 5,
        mouse_clicks: 25,
        mouse_movements: 150,
        mouse_velocity: 2.5,
        mouse_idle_time: 1000,
        scroll_depth: 0.7,
        scroll_speed: 3.2,
        scroll_events: 8,
        focus_changes: 2,
        ...
        metadata: { risk_level: user.risk_level, device_type: 'desktop', risk_score: user.risk_level === 'high' ? 85 : user.risk_level === 'medium' ? 45 : 15 }
      }));
      
      console.log('Flask API data loaded:', { 
